...
**Off-policy** (two separate policies):
- **Target policy** π: the policy we want to evaluate/improve (can be fully greedy = optimal)
- **Behavior policy** b: a different policy that generates episodes (exploratory, must cover all actions π would take)
- The key: learning about π from experience generated by b — decouples exploration from the policy being learned
- Uses **importance sampling** to correct for the distribution mismatch: weight returns by the cumulative probability ratio Π π(a|s)/b(a|s) along the trajectory
